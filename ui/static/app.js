...
function renderTrace(trace) {
  ...
  return `<section class="trace-card"><div class="trace-title"><strong>Retrieved context</strong><span>${trace.products.length} product listing</span></div>${trace.products.map((item) => `<div class="trace-row"><span class="trace-score">${Number(item.score).toFixed(3)}</span><div><strong>${escapeHtml(item.name)}</strong><p>Cosine similarity · ${escapeHtml(item.matched_fields.join(", "))}</p><code>[${escapeHtml(item.citation.chunk_id)} | ${escapeHtml(item.citation.document_id)}]</code></div></div>`).join("")}</section>`;
}
...
async function sendMessage(message) {
  ...
  try {
    ...
    await consumeSse(response, (event, data) => {
      ...
      if (event === "thinking") { runState.textContent = "Thinking · query rewrite"; }
      if (event === "rewrite") { originalQuery.textContent = data.query; rewrittenQuery.textContent = data.rewritten_query; runState.textContent = data.query_rewritten ? "Rewritten · retrieving" : "Retrieving"; }
      if (event === "retrieval") { trace = { products: data.products }; assistant.item.querySelector(".message-body").insertAdjacentHTML("beforeend", renderTrace(trace)); updateInspector({ latency_ms: "…", total_tokens: "…", cost_usd: null, retrieve_count: trace.products.length, query: data.query, rewritten_query: data.rewritten_query, top_k: data.top_k, guardrail: "passed", retrieve_where: "data/k4_asos_products", retrieval_strategy: "BGE-M3 cosine similarity" }, trace.products); runState.textContent = "Generating"; }
